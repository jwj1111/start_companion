...
| Phase | 目标 | 核心任务 | 验收标准 |
|-------|------|----------|----------|
| **1. 能说话**| 文本对话跑通 | 人设加载 → LangGraph ReAct → 用户微调 | 有人设地聊天，用户能改性格 |
| **2. 能听能看**| 多模态核心体验 | ASR/TTS + 截图分析 + 情感/Live2D | 语音聊天 + 看游戏画面 + 表情 |
| **3. 能记住 + 懂游戏**| 短期记忆 + 联网 | 摘要压缩 + Profile 档案 + 联网搜索 | 30轮不丢；跨session记称呼；答游戏问题 |
| **4. 深度记忆**| 长期记忆 RAG | 向量库 + 提取去重 + MemoryProvider + 审核API | 跨session记住偏好/事件 |
| **5. 深度游戏理解**| 游戏知识库 | 知识摄入 + game_info_tool 知识库路径 | 攻略准确度提升 |
| **6. 生产化**| 上线准备 | MySQL + 记忆管理UI + 监控 + 压测 | 多用户并发 |
